...
################################################################################
"""
BaseChartWrapper - Google Chart API Wrapper
...
Example
    ...
    >>> chart = BaseChart('lc',['simpleisbetterthancomplexcomplexisbetterthancomplicated'])
    ...
    >>> chart.image() # PIL instance
    <PngImagePlugin.PngImageFile instance at ...>
    >>> 1#chart.show() # Webbrowser open
    True
    >>> chart.save('tmp.png') # Save to disk
    'tmp.png'

See tests.py for unit test and other examples
"""
from gchart.settings import *
...
class Axes(dict):
    ...
    Use this class via BaseChart(...).axes
    ...
    """
    def __repr__(self):
        return '<BaseChart.Axes %s>' % dict(self.items())
...
class BaseChart(dict):
    ...
    def fromurl(cls, qs):
        """
        Reverse a chart URL or dict into a BaseChart instance
        
        >>> G = BaseChart.fromurl('http://chart.apis.google.com/chart?...')
        >>> G
        <BaseChartWrapper.BaseChart instance at...>
    ...
    def _parts(self):
        return ('%s=%s' % (k,smart_str(v))
                for k,v in sorted(self.items()) if v)
    ...
    def show(self, *args, **kwargs):
        ...
        return webopen(self.url, *args, **kwargs)
...
class QRCode(BaseChart):
    def __init__(self, content='', **kwargs):
        ...
        BaseChart.__init__(self, 'qr', None, **kwargs)
        
class Text(BaseChart):
...
class _AbstractBaseChart(BaseChart):
    opts = {}
    cht = None
    def __init__(self, dataset, **kwargs):
        kwargs.update(self.opts)
        BaseChart.__init__(self, self.cht, dataset, **kwargs)


class Meter(_AbstractBaseChart):
    opts = {'encoding':'text'}
    cht = 'gom'
class Line(_AbstractBaseChart):
    cht = 'lc' 
class LineXY(_AbstractBaseChart):
    cht = 'lxy' 
class HorizontalBarStack(_AbstractBaseChart):
    cht = 'bhs' 
...
class VerticalBarGroup(_AbstractBaseChart):
    cht = 'bvg' 
class Pie(_AbstractBaseChart):
    cht = 'p' 
class Pie3D(_AbstractBaseChart):
    cht = 'p3' 
class Venn(_AbstractBaseChart):
    cht = 'v' 
class Scatter(_AbstractBaseChart):
    cht = 's' 
class Sparkline(_AbstractBaseChart):
    cht = 'ls' 
class Radar(_AbstractBaseChart):
    cht = 'r' 
class RadarSpline(_AbstractBaseChart):
    cht = 'rs' 
class Map(_AbstractBaseChart):
    cht = 't' 
